...
df_st <- df_st %>%
  select(any_of(c(
    'Squad', 'Team_or_Opponent', 'Starts_Playing', 'Min_Playing', 'Gls', 'Ast', 'G+A', 'G_minus_PK',
...
df_sh <- df_sh %>%
  select(any_of(c(
    'Squad', 'Team_or_Opponent', 'Gls_Standard', 'Sh_Standard', 'SoT_Standard',
...
df_p <- df_p %>%
  select(any_of(c(
    'Squad', 'Team_or_Opponent', 'Cmp_Total', 'Att_Total', 'Cmp_percent_Total',
...
df_def <- df_def %>%
  select(any_of(c(
    'Squad', 'Team_or_Opponent', 'Tkl_Tackles', 'TklW_Tackles',
    'Blocks_Blocks', 'Int', 'Tkl+Int', 'Clr', 'Err'
  )))

df_misc <- df_misc %>%
  select(any_of(c(
    'Squad', 'Team_or_Opponent', 'Fls', 'Fld', 'Off', 'Crs',
    'Won_Aerial', 'Lost_Aerial', 'Won_percent_Aerial'
  )))

df_k <- df_k %>%
  select(any_of(c(
    'Squad', 'Team_or_Opponent', 'GA', 'GA90', 'SoTA', 'Saves', 'Save_percent',
...
merged_df <- df_st %>%
  full_join(df_sh,  by = c("Squad", "Team_or_Opponent")) %>%
  full_join(df_p,   by = c("Squad", "Team_or_Opponent")) %>%
  full_join(df_def, by = c("Squad", "Team_or_Opponent")) %>%
  full_join(df_misc,by = c("Squad", "Team_or_Opponent")) %>%
  full_join(df_k,   by = c("Squad", "Team_or_Opponent"))

# --- 5. Save outputs ---
dir.create("data", showWarnings = FALSE)

# Individual tables (optional)
write.csv(df_st,  "data/df_st_team.csv",  row.names = FALSE)
write.csv(df_sh,  "data/df_sh_team.csv",  row.names = FALSE)
write.csv(df_p,   "data/df_p_team.csv",   row.names = FALSE)
write.csv(df_def, "data/df_def_team.csv", row.names = FALSE)
write.csv(df_misc,"data/df_misc_team.csv",row.names = FALSE)
write.csv(df_k,   "data/df_k_team.csv",   row.names = FALSE)

# Unified dataset
write.csv(merged_df, "data/fbref_data_team.csv", row.names = FALSE)
